...
function describeSync(sync) {
  ...
  if (sync.status === "listing") return "Reading Drive…";
...
async function handleBrowseFolders() {
  ...
  try {
    const response = await adminFetch("/browse-google-drive-folders", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ searchTerm: elements.folderSearch.value.trim().toLowerCase() })
    });
    ...
    elements.folderBrowserResults.replaceChildren(
      ...payload.folders.map((folder) => {
        ...
        useButton.addEventListener("click", () => {
          elements.createForm.googleDriveFolderName.value = folder.name;
          elements.folderBrowserError.textContent = `'${folder.name}' copied into the create form.`;
        });
